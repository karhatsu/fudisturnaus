...
const PlayoffMatches = ({ onItemDelete, onItemSave, tournament, tournamentId }) => {
  ...
  const ageGroupsIdsWithTables = ageGroups
    .filter((ageGroup) => ageGroup.calculateGroupTables)
    .map((ageGroup) => ageGroup.id)
  const groupIdsWithTables = groups
    .filter((group) => ageGroupsIdsWithTables.includes(group.ageGroupId))
    .map((group) => group.id)
  ...
  const renderCannotAddPlayoffMatches = () => {
    return (
      <div className="tournament-item">
        Jatko-otteluiden lisääminen vaatii vähintään yhden kentän sekä vähintään kaksi joukkuetta sarjassa, jolle
        lasketaan sarjataulukot.
      </div>
    )
  }
